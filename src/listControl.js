...
Sortable.create(custList,{
    ...
    onSort: function (e) {
        ...
        //getDropCustomer(e.from.childNodes[e.newIndex].id).drop = e.from.childNodes.length - e.newIndex;
...
function unitsOnSelect(e){
    ...
    if (activeUnit != null ){
        ...
        if (activeUnit.inCanvas == true){
            ...
            //listUnits(activeUnit);
            //e.item.attributes.style.value="background-color: white; border: 1px solid #fb8302; white-space: nowrap; width: 99%;";
        }
        if (editing == false){
            updateListUnits();
        }else {
